# Jack Wilson
# 2/11/2026
# Model for pre-race predictions (practice + qualifying data)
...
def model_pre_race(from_file=True, grand_prix=None, train_new_model=True,
                   model_path=None, iterations=1000, learning_rate=0.05,
                   depth=6, l2_leaf_reg=3, verbose=False):
    """
    Train and/or use model for pre-race predictions (includes qualifying).
    ...
    model_path : str, optional
        Path to save/load model. Defaults to models/model_pre_race.cbm
    iterations : int
    ...
    if model_path is None:
        ...
        model_path = os.path.join(models_dir, "model_pre_race.cbm")
    ...
    # Prepare features
    df_features, feature_cols = prepare_features(df, data_type='pre_race')
    ...
    X_pred, _ = prepare_features(pred_data, data_type='pre_race')
